...
mod tests {
    ...
    impl Dataset for TestDataset {
        fn get(&self, index: usize) -> Option<Vec<Tensor>> {
            ...
            let tensor0 = Tensor::from_raw_buffer(
                &self.x[index],
    ...
    fn test_single_worker() {
        ...
        let mut dataloader = DataLoader::new_single_worker(dataset.clone(), false, 3, false, None);
        for epoch in 0..2 {
            println!("epoch: {}", epoch);
        ...
        let mut dataloader = DataLoader::new_single_worker(dataset.clone(), true, 3, true, None);
        for epoch in 0..2 {
            println!("epoch: {}", epoch);
            for (i, batch) in dataloader.by_ref().enumerate() {
                let y = batch[1].to_vec2::<u8>().unwrap();
    ...
    fn test_multi_worker() {
        ...
        let mut dataloader =
            DataLoader::new_multi_worker(dataset.clone(), false, 3, false, None, 5, Some(5));
        ...
        let mut dataloader =
            DataLoader::new_multi_worker(dataset, true, 3, false, Some(333), 5, Some(5));
        for epoch in 0..2 {
            println!("epoch: {}", epoch);
            for (i, batch) in dataloader.by_ref().enumerate() {
                let y = batch[1].to_vec2::<u8>().unwrap();
